...
class Pagination
{
     ...
     * @return
     */
    private function generateHtml($page, $text = '')
    {
        $address = $this->router->getParsedURI()['address'];
        $index = $this->index;
        $params = $paramsCount = '';
        if(!empty($this->router->getGet()))
        {
            $params = $this->router->getGet()['param_string'];
            $paramsCount = $this->router->getGet()['count'];
        }
        ...
        $params = preg_replace('~'.$index.'[+-]?[0-9]+~', '', $params);
        if(!empty($params))
        {
            $params = trim($params, '&');
            if($paramsCount >= 2)
        ...
        //var_dump('2: '.$currentURI);
        # Формируем HTML код ссылки и возвращаем
        return
                '<li><a href="/'.$address.'?'.$params.$index.$page.'">'.$text.'</a></li>';
    }
